...
def on_message(client, userdata, msg):
    ...
    if msg.topic == TOPIC_STATUS:
        ...
        if payload == "MATCH":
            ...
            try:
                requests.post("http://127.0.0.1:5000/api/log/fingerprint", 
                            json={"status": "MATCH"})
            except Exception as e:
                print(f"[API] Error: {e}")
            ...
            enroll_status.update({"status": "step1", "pesan": "👆 Tempelkan jari (1/2)"})
            print("[MQTT] Enroll step 1")
            
        elif payload.startswith("ENROLL_STEP2"):
            enroll_status.update({"status": "step2", "pesan": "👆 Tempelkan jari sama (2/2)"})
